...
public class FruitBlastGameEngineTests {
    ...
    private readonly FruitBlastGameEngine _engine;

    public FruitBlastGameEngineTests() {
        ...
        _repo.Setup(r => r.GetGameByType("FruitBlast")).Returns(new Game { Id = Guid.NewGuid(), Name = "Fruit Blast" });
        _repo.Setup(r => r.BeginTransaction()).Returns(new Mock<ITransaction>().Object);

        _engine = new FruitBlastGameEngine(_rng.Object, _vault.Object, _brain.Object, _promo.Object, _jackpot.Object, _realTime.Object, _scopeFactory.Object, _cache.Object, _lock.Object);
    }

    [Fact]
    public async Task GetCurrentState_ShouldReturnValidState() {
        var sessionId = Guid.NewGuid();
        _repo.Setup(r => r.GetLastRound(sessionId)).Returns((GameRound)null!);
        _repo.Setup(r => r.GetSessionAsync(sessionId)).ReturnsAsync(new GameSession { Id = sessionId, UserId = Guid.NewGuid() });
        _repo.Setup(r => r.GetOrCreateLocalJackpot(It.IsAny<Guid>())).Returns(new Jackpot { CurrentValue = 100 });
